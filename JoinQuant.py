...
class JoinQuant:
    ...
    LOGIN_PAGE = WEB_ROOT
    LOGIN_API =  '{}/user/login/doLoginByText'.format(WEB_ROOT)
    ...
    DOWNLOAD_FILES = WEB_ROOT + '/user/{}/files/{}?download=1'
    ...
    def __init__(self, config):
        ...
        self.data_path = os.path.join(self.script_dir, '')
    ...
    def fetch_file(self, file_name, new_file_name):
        # account_config = self.config['account']
        print(self.DOWNLOAD_FILES.format('23286199053', file_name))
        rep = self.s.get(self.DOWNLOAD_FILES.format('23286199053', file_name))
...
class JoinQuantTest(unittest.TestCase):
    ...
    def test_downlad_file(self):
        self.jq.fetch_file('trade.txt', 'trade.txt')
