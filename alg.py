...
#def before_trading_start(context):
    ...
    if g.if_trade and not g.trading_postponed and len(g.feasible_stocks): 
...
def set_factors(context, is_hl_win):
    ...
    elif g.days_since_last_buy > 90 and g.index_fall_a_lot:
        g.factors = {
            'P': (t.PStockFactor, {'data_type': 'price', 'prices_len': 1}, {}, {'days': 1, 'asc': True})
        }
...
def set_feasible_stocks(context):
    ...
    if False and current_month in g.forbidden_month:
        ...
        print("days above 39 ma in last 90 days: " + str((index_prices.ix[-90:, 0] > index_ma.ix[-90:]).sum()))
        if (index_prices.ix[-90:, 0] > index_ma.ix[-90:]).sum() < 20:
            g.is_bear_market = True
        elif g.is_bear_market:
        ...
        use_low_price = False
        if g.days_since_last_buy > 90 and g.index_fall_a_lot:
            use_low_price = True
...
def trade(context):
    ...
    if not g.if_trade and g.spring_festival_buy and len(context.portfolio.positions.keys()) < g.min_stocks_to_buy:
        schduler = t.SpringFestivalTradeScheduler(g, context, lambda: set_feasible_stocks(context), 3)
        schduler.trade()
