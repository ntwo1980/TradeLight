...
class JoinQuantWeekdaylyStatJob(b.BlogPostGenerateJobBase):
    def __init__(self, post_path, data_file_path):
        b.BlogPostGenerateJobBase.__init__(self, post_path)
        self.data_file_path = data_file_path

    def run(self):
        blog_generator = HexoGenerator.HexoGenerator(self.post_path, '主要指数周内日回报统计 - {}'.format(j.JobBase.get_today_str()), tags=['数据统计'])
