...
trade_day = None
use_hl_index = None
fundamental_selector_config = {'mc': (0, 0.8), 'pb': (0, 0.4), 'iop': (0.6, 1), 'roic': (0.6, 1), 'rp': (0, 1)}
xp_fundamental_selector_config1 = {'mc': (0, 0.35), 'pe': (0, 0.9), 'pb': (0, 0.4) ,'ps': (0, 0.4), 'iop': 15, 'facr': (0, 1.1)}
xp_fundamental_selector_config2 = {'mc': (0, 0.35), 'pe': (0, 0.9), 'pb': (0, 0.4) ,'ps': (0, 0.4), 'iop': (0.65, 1), 'facr': (0, 1.1)}
treat_index_fall_a_lot = False
treat_stop_lose = False
sell_stock = None
buy_stock = None
use_hl_index = None
...
class MaTradeScheduler(TradeScheduler):
    def __init__(self, g, context, trade_func, index, max_below_ma):
        TradeScheduler.__init__(self, g, context, trade_func)
        self.index = index
        self.max_below_ma = max_below_ma
    ...
    def need_trade(self):
        ...
        below_ma_days = (index_prices.ix[-90:] < index_ma.ix[-90:]).sum()

        return index_prices[-1] > index_ma[-1] and below_ma_days < self.max_below_ma
...
class SpringFestivalTradeScheduler(TradeScheduler):
    ...
    def need_trade(self):
        ...
        if self.offset > 0:
            ...
            return scheduled_trade_day <= today <= sprint_festival.date()
        else:
            return False
...
class NotNewSecuritySelector(SecuritySelector):
    def __init__(self, stocks, date, days):
        past_stocks = get_all_securities(date=shift_trading_day(date, -days))

        self.stocks = list(set(stocks).intersection(set(past_stocks.index)))

class NewSecuritySelector(SecuritySelector):
    def __init__(self, date, days):
        past_stocks = get_all_securities(date=shift_trading_day(date, -days))
        current_stocks = get_all_securities(date=date)

        self.stocks = [s for s in current_stocks.index if s not in past_stocks.index]
...
class NotBlackListSecuritySelector(SecuritySelector):
    def __init__(self, stocks):
        blacklist = {'002087.XSHE', '600713.XSHG'}
...
class PEHistoryRankSecuritySelector(SecuritySelector):
    def __init__(self, stocks, date, max):
        ...
        stocks = fundamentals['code']
        results = []

        for chunk in chunks(stocks, 10):
            df = get_fundamentals_continuously(query(valuation.pb_ratio
                ).filter(
                    indicator.code.in_(chunk)
                ), end_date=date, count=720)
            df = df.xs('pb_ratio',axis=0)
            df = df.rank(ascending = True, method = 'max', pct=True).iloc[-1]

            results.extend(df[df<max].index)
...
class FundamentalSecuritySelector1(SecuritySelector):
    def __init__(self, stocks, date, mc, pb, iop, roic):
        ...
        fundamentals['roe_r'] = fundamentals['roe'].rank(ascending = True, method = 'max', pct=True)
...
class FundamentalSecuritySelector3(SecuritySelector):
    def __init__(self, stocks, date, mc, roe, iop, roic, gp):
        fundamentals = get_fundamentals(query(valuation.code,
                indicator.inc_operation_profit_year_on_year,
                indicator.gross_profit_margin,
                indicator.roe,
        ...
        fundamentals = fundamentals[(fundamentals['mc_r'] >= mc[0]) \
                                & (fundamentals['mc_r'] <= mc[1]) \
                                & (fundamentals['roe_r'] >= roe[0]) \
                                & (fundamentals['roe_r'] <= roe[1]) \
                                & (fundamentals['iop_r'] >= iop[0]) \
                                & (fundamentals['iop_r'] <= iop[1]) \
                                & (fundamentals['gpm_r'] >= gp[0]) \
                                & (fundamentals['gpm_r'] <= gp[1]) \
                                & (fundamentals['ROIC_r'] >= roic[0]) \
                                & (fundamentals['ROIC_r'] <= roic[1])]
        fundamentals = fundamentals.set_index(['code'])

        self.stocks = set(fundamentals.index)
...
class LongUpSecuritySelector(SecuritySelector):
    def __init__(self, date, day_count, pct):
        stocks = get_all_securities(types=['stock'])
        past = (date - dt.timedelta(day_count + 10))
        stocks = stocks[(stocks['end_date'] > date)&(stocks['start_date']<=past)]
        prices = get_price(list(stocks.index), end_date=today, count = day_count, fields=['close'])['close']
        ...
        df = pd.DataFrame({'positive_count': stat})
        df = df.head(int(len(stocks) * pct))
        self.stocks = list(df.index)
...
class PegSecuritySelector(SecuritySelector):
    def __init__(self, stocks, date, peg, max_pe, strict_limit):
        if strict_limit:
        ...
        if strict_limit and len(fundamentals.index) < 30:
            fundamentals1 = get_fundamentals(query(
                    indicator.code,
                    indicator.inc_net_profit_to_shareholders_year_on_year,
                    indicator.inc_revenue_year_on_year,
                    valuation.pe_ratio
            ...
            if len(fundamentals1.index) > 60:
                fundamentals = fundamentals1

        fundamentals.loc[fundamentals.inc_net_profit_to_shareholders_year_on_year < 0, 'inc_net_profit_to_shareholders_year_on_year'] = 0.1
...
class FallSecuritySelector(SecuritySelector):
    def __init__(self, stocks, start_date, end_date, fall):
        df_price = get_price(stocks,
            start_date = start_date,
            end_date=end_date,
            frequency='daily',
            fields='close')['close']

        df_fall = pd.DataFrame({'Fall': (df_price.iloc[-1] - df_price.iloc[0]) / df_price.iloc[0]})
        df_fall['rank'] = df_fall['Fall'].rank(ascending = True, method = 'max', pct=True)

        df_fall = df_fall.dropna()
        df_fall = df_fall[(df_fall['rank'] <= fall)]

        self.stocks = list(df_fall.index)

class PBHistorySelector(SecuritySelector):
    def __init__(self, stocks, date, day_count, pb):
        stocks = set_feasible_stocks(stocks, day_count, date)
        past = shift_trading_day(date, -day_count)
        past_stocks = get_all_securities(date=shift_trading_day(date, -day_count))
        stocks = list(set(stocks) & set(past_stocks.index))

        fundamentals_list = []
        for i in range(0, int(day_count/21)):
            statDate = shift_trading_day(date, -21 * i)
            fundamentals = get_fundamentals(query(valuation.code,
                valuation.pb_ratio,
            ).filter(
                indicator.code.in_(stocks)
            ...
            fundamentals = fundamentals.set_index(['code'])
            fundamentals.columns = ['pb']
            fundamentals_list.append(fundamentals)

        df_stat = pd.DataFrame()
        sum = fundamentals_list[0]['pb']
        for j in range(1, len(fundamentals_list)):
            sum = sum + fundamentals_list[j]['pb']
            df_stat['pb_pct'] = fundamentals_list[0]['pb'] / (sum / len(fundamentals_list))

        df_stat['rank'] = df_stat['pb_pct'].rank(ascending = True, method = 'max', pct=True)
        df_stat = df_stat[(df_stat['rank'] <= pb)]

        self.stocks = list(df_stat.index)
...
class VarStockFactor(PriceStockFactor):
    def generate(self, days, asc):
        ...
        df_var = pd.concat([df_var.ix[:-3,], df_var.ix[-3:,] * 1.5])
...
class WeightVarStockFactor(PriceStockFactor):
    def generate(self, days, weight_days, weight_factor, asc):
        df_var = self.prices[-(days+1):].pct_change()
        df_var = pd.concat([df_var.ix[:-weight_days,], df_var.ix[-weight_days:,] * weight_factor])

        # 进行转置
        df_var = df_var.T
        # 生成方差
        df_var['WeightVar'] = df_var.var(axis = 1, skipna = True)
        # 生成新的DataFrame
        df_var = pd.DataFrame(df_var['WeightVar'])
        # 删除nan
        df_var = df_var.dropna()
        # 排序给出排序打分
        df_var['WeightVar_sorted_rank'] = df_var['WeightVar'].rank(ascending = asc, method = 'dense')
