...
class FundamentalSecuritySelector1(SecuritySelector):
    def __init__(self, stocks, date, mc, pb, iop, roic):
        ...
        if date.month > 3 and date.month < 8 and False:
            negative_roic_stocks = fundamentals[fundamentals['ROIC'] < 0].index
            fundamentals_roic = get_fundamentals(query(valuation.code,
                income.net_profit,
                income.income_tax_expense,
                income.financial_expense,
                balance.total_owner_equities,
                ...
                indicator.code.in_(negative_roic_stocks)
            ), str(date.year) + '-4-1')
            fundamentals_roic.fillna(0, inplace=True)
            fundamentals_roic['ROIC'] = ((fundamentals_roic['net_profit'] + fundamentals_roic['income_tax_expense'] + fundamentals_roic['financial_expense'])
                    / (fundamentals_roic['total_owner_equities'] + fundamentals_roic['shortterm_loan'] + fundamentals_roic['longterm_loan'] +
                      fundamentals_roic['non_current_liability_in_one_year'] + fundamentals_roic['bonds_payable'] +  fundamentals_roic['longterm_account_payable']))
            fundamentals_roic = fundamentals_roic.set_index(['code'])
            for stock in negative_roic_stocks:
                if stock in fundamentals_roic.index:
                    fundamentals.ix[stock,'ROIC'] = fundamentals_roic.ix[stock,'ROIC']
            fundamentals['ROIC_r'] = fundamentals['ROIC'].rank(ascending = True, method = 'max', pct=True)
        ...
        #print(fundamentals1.join(prices.T))

        #self.stocks = set(fundamentals1.index) | set(fundamentals2.index)
        self.stocks = list(fundamentals1.index)
...
def get_turning_point(prices, window = 0):
    ...
    df = pd.concat([prices,
               prices.ix[:,-1:].rolling(double_window).apply(lambda x: pd.Series(x).min()),
               prices.ix[:,-1:].rolling(double_window).apply(lambda x: pd.Series(x).argmin()),
               prices.ix[:,-1:].rolling(double_window).apply(lambda x: pd.Series(x).max()),
               prices.ix[:,-1:].rolling(double_window).apply(lambda x: pd.Series(x).argmax())
             ], axis=1)
