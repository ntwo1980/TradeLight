...
class SimpleGridStrategy(BaseStrategy):
    ...
    def f(self, C):   # SimpleGridStrategy
        ...
        if self.current_price >= base_price + self.grid_unit * 1.000001:
        ...
        if self.IsBacktest:
            ...
            if account.m_dBalance - account.m_dAvailable > universe.max_position:
                universe.max_position = account.m_dBalance - account.m_dAvailable
            self.Print(f"Total position={universe.max_position}")
    ...
    def ExecuteSell(self, C, stock, current_price, current_holding):   # SimpleGridStrategy
        ...
        if self.NetValue > 0:
            self.Print(f"premium rate {current_price / self.NetValue}")
        if self.NetValue > 0 and current_price / self.NetValue > 1.05 and current_holding <= unit_to_sell * 2:
    ...
    def GetGridUnit(self, stock, price, atr):   # SimpleGridStrategy
        ...
        return max(atr * 0.5, price * self.minUnitPercentage)
